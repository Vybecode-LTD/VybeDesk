...
>
> **STATE 2026-05-29 — v0.32 IN PROGRESS, NO OPEN BUGS.**
...
## Last Completed Task
**v0.32 (in progress — 2026-05-29): Both bugs RESOLVED + Codex
audit fix plan tasks 1–9 done + cascade delete tests + doc cleanup.**

The 2026-05-28 session (Codex-audit-driven) resolved both the
cross-module project persistence bug and the layout regression bug,
added "choose a project" landing overlays on all 6 project-scoped
modules, and added Notebook landing pagination (4 per page). The
2026-05-29 session continued the audit fix plan: cascade delete tests
(10 new), doc/narrative cleanup, test count updates. Build green;
207/207 tests pass. No remaining open bugs.

**Persistence bug root cause:** passive null writes from TwoWay
ComboBox bindings flowing through `ActiveProjectContext.SetCurrent`.
Fixed with: (1) idempotent null-safe `SetCurrent` (null no-ops,
same-ID no-ops, explicit `ClearCurrent()`), (2) per-module project
isolation hardening (`_lastSelectedProjectId`, `_reloadingProjects`,
null-write suppression, `OnActivated()` restore).

### RESOLVED — Layout regression (2026-05-28)

Root cause: Fluent ContentControl defaulting `VerticalContentAlignment`
to `Top` (infinite height measure). Fix:
...
### RESOLVED — Cross-module project persistence (2026-05-28)

Root cause: passive null writes from TwoWay ComboBox bindings through
`ActiveProjectContext.SetCurrent`. Fixed with idempotent null-safe
`SetCurrent` + explicit `ClearCurrent()` + per-module project isolation
hardening. See
[docs/PROJECT_PERSISTENCE_BUG.md](docs/PROJECT_PERSISTENCE_BUG.md)
...
- **Cross-module project persistence — RESOLVED (v0.32, 2026-05-28).**
  Root cause was passive null writes from TwoWay ComboBox bindings
  through `ActiveProjectContext.SetCurrent`. Fixed with idempotent
  null-safe `SetCurrent` + per-module isolation. See
  [docs/PROJECT_PERSISTENCE_BUG.md](docs/PROJECT_PERSISTENCE_BUG.md)
  for the full postmortem.
...
- @docs/PROJECT_PERSISTENCE_BUG.md — **RESOLVED postmortem (2026-05-28)**;
  cross-module project selection persistence. Reference for understanding
  the `ActiveProjectContext` pattern and its null-safety invariants.
