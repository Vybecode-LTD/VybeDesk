...
public sealed partial class PromptManagerViewModel : PageViewModel
{
    ...
    /// <summary>
    /// Active project filter. When null or <see cref="AllProjectsSentinel"/>,
    /// the prompts list is unfiltered. When a real project is selected, only
    /// prompts whose <c>Tags</c> contain the project's name (case-insensitive)
    /// are shown — the same tag scheme written by IProjectImportService.
    /// </summary>
    [ObservableProperty] private Project? _selectedProject = AllProjectsSentinel;
    ...
    partial void OnSelectedProjectChanged(Project? value) => _ = ApplyFilterAsync();
