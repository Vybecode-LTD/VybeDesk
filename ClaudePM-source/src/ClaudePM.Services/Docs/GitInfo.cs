...
internal static class GitInfo
{
    ...
    /// <summary>
    /// Count of commits to the repo HEAD between <paramref name="since"/> and
    /// now. Null on missing git binary, non-repo folder, or any failure.
    /// Used by Home's project health card to show "X commits in the last 7
    /// days" without parsing full git log output.
    /// </summary>
    public static async Task<int?> GetCommitCountSinceAsync(
        string repoRoot, DateTimeOffset since, CancellationToken ct)
    {
        var psi = new ProcessStartInfo("git")
        {
            WorkingDirectory = repoRoot,
        ...
        psi.ArgumentList.Add("--since=" + since.UtcDateTime.ToString("o"));
