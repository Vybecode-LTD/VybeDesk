...
Vision Audit (v0.30 — drift detector with persisted audit history). v0.31
added an app-wide UI cohesion pass: unified `ModuleHeader` on every page,
sidebar `TreeView` with Skills expanded to Manager + Builder children,
Documentation tab restructured to match the Skill Manager layout, and
global always-visible thin scrollbars with a 50 px content gap. Highlights: safety
...
a unified 105px `ModuleHeader`. The sidebar `ListBox` was replaced with
a `TreeView` — Skills appears as an expandable parent with Manager +
Builder as nested children. App opens **Maximized** on startup.
...
- **Skills** — folder-format Claude skill manager (rebuilt v0.28; the
  ...
  skill. The Section's sidebar TreeView item expands to reveal a
  **Skill Builder** sub-page (Phase 2, shipped v0.29).
