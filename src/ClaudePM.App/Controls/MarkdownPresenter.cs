...
namespace ClaudePM.App.Controls;
...
public sealed class MarkdownPresenter : ContentControl
{
    ...
    private Border RenderTable(Table table)
    {
        ...
        // Star-weight each column by the longest text length across its
        // cells, so e.g. a 2-col table with short labels on the left and
        // long prose on the right doesn't waste half the width on labels.
        var weights = new int[maxCols];
        ...
        // Star columns in Avalonia don't grow to fit children — they take
        // their proportional share. So we set a per-column MinWidth sized
        // to the header text so headers always fit single-line. Estimated
        // px per char for the SemiBold header at ~13px font + cell padding.
        const double headerPxPerChar = 8.5;
        const double cellPaddingX    = 16;
        for (int c = 0; c < maxCols; c++)
        {
            var weight = Math.Max(1, weights[c]);
