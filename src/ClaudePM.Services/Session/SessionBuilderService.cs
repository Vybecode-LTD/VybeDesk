...
public sealed class SessionBuilderService : ISessionBuilderService
{
    ...
    public async Task<string> GenerateAsync(SessionPlan plan, CancellationToken ct = default)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(root, "CLAUDE.md"), BuildClaudeMd(plan), ct);
        await File.WriteAllTextAsync(Path.Combine(root, "README.md"), BuildReadme(plan), ct);
        await File.WriteAllTextAsync(Path.Combine(root, ".gitignore"), GitignoreText, ct);
        await File.WriteAllTextAsync(Path.Combine(root, "KICKOFF.md"), BuildKickoff(plan), ct);
    ...
    private static string BuildClaudeMd(SessionPlan p) =>
        "# CLAUDE.md — " + p.ProjectName + "\n\n" +
        ...
        "## Overview\n" + Or(p.Description, "<To be defined.>") + "\n\n" +
        "## Architecture\n<To be defined — see docs/transcripts/ for the design discussion.>\n\n" +
        "## Build, Test, Run\n<To be defined.>\n\n" +
        "## Stack\n" + Or(p.Stack, "<To be defined.>") + "\n\n" +
        "## Reference Docs\n" +
        "The original claude.ai conversation(s) are in docs/transcripts/. See KICKOFF.md\n" +
        "for the first task.\n";

    private static string BuildReadme(SessionPlan p) =>
        "# " + p.ProjectName + "\n\n" +
        Or(p.Description, "_No description provided._") + "\n\n" +
        "## Stack\n" + Or(p.Stack, "TBD") + "\n\n" +
        "## Status\n" +
        "Handed off from claude.ai via ClaudePM. See CLAUDE.md for context and KICKOFF.md\n" +
        "to begin work in Claude Code.\n";

    private static string BuildKickoff(SessionPlan p) =>
        "# Kickoff — " + p.ProjectName + "\n\n" +
        Or(p.Description, "_No description provided._") + "\n\n" +
        "## Context\n" +
        "Full context is in CLAUDE.md. The original claude.ai conversation(s) that shaped\n" +
        "this project are in docs/transcripts/ — read those first.\n\n" +
        "## First task\n" +
        "Review CLAUDE.md and docs/transcripts/, confirm the architecture and any open\n" +
        "questions below, then scaffold the project structure.\n\n" +
        "## Open questions\n" +
        "<List anything the transcripts left unresolved. Confirm these before building\n" +
        "where they would block progress.>\n\n" +
        "## Staged files\n" +
        "Any files brought over from the chat are in the package root.\n";

    private const string GitignoreText =
        "bin/\nobj/\nbuild/\ndist/\nnode_modules/\n.vs/\n.idea/\n*.user\n.DS_Store\nThumbs.db\n";
}
