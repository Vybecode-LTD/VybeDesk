...
public sealed partial class NotebookViewModel : PageViewModel
{
    ...
    /// For Notebook this is named _lastActiveProjectId (not _lastSelectedProjectId)
    /// because the Notebook uses ActiveProject, not SelectedProject.
    /// </summary>
    private Guid? _lastActiveProjectId;
    ...
    public bool HasProject => ActiveProject is not null;

    /// <summary>
    /// Projects excluding the "All Projects" sentinel, for the landing page
    /// project cards. The sentinel gets its own prominent button.
    /// </summary>
    public IEnumerable<Project> LandingProjects =>
        Projects.Where(p => !IsAllProjects(p));

    // ── Landing-page pagination (4 projects per page) ──────────────
    ...
    public int LandingTotalPages =>
        Math.Max(1, (int)Math.Ceiling(LandingProjects.Count() / (double)LandingPageSize));

    public bool LandingHasMultiplePages => LandingTotalPages > 1;
    public string LandingPageLabel => $"{LandingCurrentPage + 1} / {LandingTotalPages}";
    public bool CanLandingPrev => LandingCurrentPage > 0;
    public bool CanLandingNext => LandingCurrentPage < LandingTotalPages - 1;

    public IEnumerable<Project> PagedLandingProjects =>
        LandingProjects.Skip(LandingCurrentPage * LandingPageSize).Take(LandingPageSize);
    ...
    {
        // Clamp page to valid range after projects change
        if (LandingCurrentPage >= LandingTotalPages)
            LandingCurrentPage = Math.Max(0, LandingTotalPages - 1);
        OnPropertyChanged(nameof(PagedLandingProjects));
        OnPropertyChanged(nameof(LandingTotalPages));
        OnPropertyChanged(nameof(LandingHasMultiplePages));
        OnPropertyChanged(nameof(LandingPageLabel));
        OnPropertyChanged(nameof(CanLandingPrev));
        OnPropertyChanged(nameof(CanLandingNext));
    }

    /// <summary>
    /// Called from the "Choose a project" landing page when the user clicks
    /// a project card or the "All Projects" button. Sets <see cref="ActiveProject"/>
    /// which hides the landing overlay and shows the normal Notebook UI.
    /// </summary>
    [RelayCommand]
    private void PickProjectFromLanding(Project? project)
    {
        if (project is not null) ActiveProject = project;
    }
    ...
    /// <summary>
    /// Re-sync ActiveProject from this module's own local memory on
    ...
    /// causes the ComboBox TwoWay binding to null out the backing field.
    /// Uses <see cref="_lastActiveProjectId"/> instead of
    /// <see cref="IActiveProjectContext.Current"/> so each module keeps
    /// its own independent project selection. The Notebook uses its own
    /// "All Projects" sentinel (Guid.Empty), so we handle that as a
    /// valid ID to restore.
    /// </summary>
    public override void OnActivated()
    {
        // First visit — no project ever selected in this module.
        // Show the "Choose a project" landing page with project cards.
        if (_lastActiveProjectId is null) return;

        if (ActiveProject?.Id == _lastActiveProjectId) return;
        var found = Projects.FirstOrDefault(p => p.Id == _lastActiveProjectId);
    ...
    private async Task RefreshScopedRootsAsync()
    {
        ...
        var keepId = ActiveProject?.Id ?? _lastActiveProjectId;
        ...
        foreach (var p in withFolders)
            Projects.Add(p);
        OnPropertyChanged(nameof(LandingProjects));
        RefreshLandingPagination();
        ...
        // Explicitly save to module-local memory so the value survives
        // even if OnActiveProjectChanged was suppressed during the rebuild.
        _lastActiveProjectId = ActiveProject?.Id;
    }
    ...
    partial void OnActiveProjectChanged(Project? oldValue, Project? newValue)
    {
        ...
        // TwoWay binding fires null back. We must NOT save/clear the
        // conversation for this — OnActivated will restore the selection
        // when we come back.
        ...
        _lastActiveProjectId = newValue?.Id;
        ...
        if (IsAllProjects(newValue))
        {
            ...
            // "All Projects" is a Notebook-internal sentinel that means
            // "scope the agent to every registered folder". It does NOT mean
            // "no project is active for the rest of the app". Publishing null
            // here was wiping the selection in every other module whenever
            // the Notebook defaulted to its sentinel at startup or the user
            // explicitly chose it. We simply don't touch IActiveProjectContext
            // in this branch — other modules keep whatever they had.
        }
        else
        {
    ...
    // ─── auto-apply (no user interaction) ────────────────────────────────
