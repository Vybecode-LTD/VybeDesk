...
namespace VybeDesk.Services.Vision;
...
public sealed class VisionAuditService : IVisionAuditService
{
    ...
    private static string ReadBoundedFileBundle(string root, IReadOnlyList<string> relativePaths)
    {
        ...
        {
            // Validate the AI-selected path stays within the project root.
            // The AI picks relative paths, but a malicious or confused
            // response could include "../../../etc/passwd" or a rooted
            // absolute path. TryResolveProjectFile rejects both.
            if (!TryResolveProjectFile(canonicalRoot, rel, out var full))
                continue;
    ...
    /// <summary>
    /// Validates that a relative path resolves to a location inside the
    /// project root. Rejects rooted paths, ".." traversal escapes, and
    /// any path that after canonicalization falls outside <paramref name="projectRoot"/>.
    /// </summary>
    internal static bool TryResolveProjectFile(string projectRoot, string relativePath, out string fullPath)
    {
        fullPath = "";
        ...
        // The resolved path must be strictly under the root (not the root itself).
    ...
    {
        using var doc = JsonDocument.Parse(JsonExtractor.ExtractJsonBlockOrThrow(raw));
