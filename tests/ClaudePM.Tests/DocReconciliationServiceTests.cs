...
public sealed class DocReconciliationServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task AuditAsync_MalformedJsonReturnsEmptyReport()
    {
        const string canned = "{\"design\":\"oops\",\"roadmapItems\":[{not valid json";

        var report = await Audit(canned);

        Assert.Same(ProjectAuditReport.Empty, report);
    }

    [Fact]
    public async Task AuditAsync_NoJsonAtAllReturnsEmptyReport()
    {
