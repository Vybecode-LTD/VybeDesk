...
public class AgentActionServiceTests
{
    ...
    public void Validate_RejectsPathOutsideScopedRoots()
    {
        ...
        var outside = new AgentAction
        {
            ...
            Path = Path.Combine(Path.GetTempPath(), "vybedesk-outside-" + Guid.NewGuid()),
        };
    ...
    public void Validate_RejectsTraversalEscape()
    {
        ...
        {
            Kind = AgentActionKind.CreateFolder,
            Path = Path.Combine(root, "..", "vybedesk-escaped"),
        };
    ...
    public void ReadFile_BlocksPathOutsideScope()
    {
        ...
        var root = Directory.CreateTempSubdirectory().FullName;
        svc.SetScopedRoots(new[] { root });
        var outside = Path.Combine(Path.GetTempPath(), "vybedesk-out-" + Guid.NewGuid() + ".txt");
    ...
    // ─── v0.32 undo/redo path confinement ──────────────────────────────

    [Fact]
    public async Task UndoLast_RefusesWhenPathNoLongerInscopedRoots()
    {
        // Execute while root A is scoped, then change roots to root B.
        // Undo should silently refuse (the entry's path is now outside
        // the sandbox). The file must NOT be deleted.
        var svc = NewSvc(out _);
        var rootA = Directory.CreateTempSubdirectory().FullName;
        var rootB = Directory.CreateTempSubdirectory().FullName;
        svc.SetScopedRoots(new[] { rootA });
        svc.SetActiveProject(Guid.NewGuid());

        var target = Path.Combine(rootA, "confined.txt");
        await svc.ExecuteAsync(new AgentAction
        {
            Kind = AgentActionKind.CreateFile,
            Path = target,
        ...
        // Swap scoped roots — rootA is no longer valid
        svc.SetScopedRoots(new[] { rootB });
        await svc.UndoLastAsync();

        // The file should still exist (undo was blocked)
        Assert.True(File.Exists(target));
    ...
    [Fact]
    public async Task RedoLast_RefusesWhenPathNoLongerInscopedRoots()
    {
        var svc = NewSvc(out _);
        var rootA = Directory.CreateTempSubdirectory().FullName;
        var rootB = Directory.CreateTempSubdirectory().FullName;
        ...
        var target = Path.Combine(rootA, "redo-confined.txt");
        await svc.ExecuteAsync(new AgentAction
        {
            Kind = AgentActionKind.CreateFile,
            Path = target,
        ...
        // The file should NOT have been re-created (redo was blocked)
        Assert.False(File.Exists(target));
